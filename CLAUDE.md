...
| **Release & delivery — the offline hand-off bundle** | ✅ **Code shipped** (`scripts/release.sh` + `scripts/release/{README.md,run.sh,run.ps1}`): a saved Docker image plus self-detecting launchers in one directory, so a recipient with only Docker Desktop — no repository access, no registry — can run the app. **The roadmap item is a retro stub with no `STATUS.md` ledger**, and `planning/user-guide/DC-22-packaging-and-deployment/` does not yet mention the bundle; retiring it means writing the bundle into DC-22 first. Item: [`planning/Roadmap/RM-19-release/`](./planning/Roadmap/RM-19-release/). |
