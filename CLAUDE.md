...
| **UX overhaul — one shell, one grammar, applied to every view + the rebuilt Compare Workspace** | ✅ Built on `ux/integration` (all 34 WPs, Phases 0–5). One page shell (PageShell/PageHeader + scroll contract S22), one tab shell (TabPanel, stable strip), one status vocabulary (StatusBadge/`lib/status`), one modal system (4 dialog tiers), one form kit (`components/form/*`), one table recipe (TableToolbar + `lib/table` pinning/sticky); master-detail views (Servers/Scans/Skills/Collections) unified via the AppShell `fullBleed+secondaryContent` variant (D-UX14); workflow cross-links (scan Δ + diff-vs-previous, console turn/error/trace links, skills usage + test-this-skill, operational dashboard, launcher cost preview via additive `GET /api/estimate/run-plan`); **Compare Workspace rebuilt per audit §H** (letter-chip run identity + URL state, Summary Δ-matrix + verdict sentences, Flow LCS trace-diff + lenses, lossless drill drawer, change markers + next-steps, suite compare) — the efficiency radar deleted. Gate green throughout (web tests 68→254, API 851→867). Plan + authoritative ledger: [`planning/Roadmap/RM-30-ux-overhaul/`](./planning/Roadmap/RM-30-ux-overhaul/) ([`STATUS.md`](./planning/Roadmap/RM-30-ux-overhaul/STATUS.md), [`verification-report.md`](./planning/Roadmap/RM-30-ux-overhaul/verification-report.md)). Source of findings: `UI-UX-AUDIT-2026-07-05.md` — **never committed to this repository**; the findings it carried survive as the WP specs and the `verification-report.md` in the item folder. **Owner-acceptance walk pending** (two-theme + keyboard + shell walks + a real compare-workspace decision; provider-key-only checks — live run-console replay, skill Trace, model rosters, live cost preview — listed in the ledger's Owner-acceptance section). Not yet merged to `main` (owner merges `ux/integration → main`) — **that sentence is now doubtful and was NOT re-verified: `main` contains no `ux/integration` merge and no `wp/ux/*` commit, yet the form kit and the rest of the Phases 0–5 grammar are on it. Treat the branch claim as unconfirmed provenance, not fact.** **The item did not end at Phase 5.** A **Phase 6** and an owner-directed **Phase 7 (Skill Studio)** were added after the audit, and Phase 7's canvas/trace/decoration WPs (7.2 · 7.3a · 7.5 · 7.6 · 7.R2a) were built direct-to-tree. **WP 7.1 shipped to `main` 2026-08-21**: a full-viewport Skill Studio at `/skills/:skillId/studio` — slim non-scrolling toolbar (Exit · Flow\|Code\|Split · Problems · dirty · Save as vN), collapsible files/tools/settings rail, collapsible context panel, bottom problems strip, and a centre surface **measured at 61.1% of a 1600×1000 viewport with both rails open** (70.5% / 79.9% collapsed), in both themes, in a real browser. The Inspector's Design tab became a read-only preview + "Edit in Studio" and its header save cluster is gone — **but two save surfaces deliberately remain** (Overview's "Save as new version", Files' "Discard / Save…"), because deleting them before WP 7.3's draft store and WP 7.4's editable files exist would remove the only way to edit a skill's keywords and files; the debt is recorded on those two WPs. **No human has used the Studio** — every visual claim is a headless-Chromium measurement — it was never exercised against a bound MCP server, and no save was ever completed. **WP 7.3, 7.4 and 7.7 have since shipped to `main`.** 7.3 gave the Studio a settings panel and **one draft store** (name · description · servers · keywords · `/command` entry points as form controls, one dirty count, one "Save as vN"), closing D-UX18; 7.4 brought files into that same draft and **deleted** the Inspector's Save/Discard bar with a guardrail against its return; **7.7 (2026-08-22)** replaced the Tools palette with a **Components palette** — nine draggable skill components plus a collapsible **MCP Servers** section that absorbs the binding chips — and deleted the "Add command", "Add section" and Legend toolbar controls, so creation has exactly one path. Each palette row is draggable **and** carries a labelled add button, so the keyboard path survives the toolbar deletion; the drop path validates an untrusted drag payload (proved by a mutation probe that reddens the `__proto__` case). One deliberate mismatch ships with it: the rail tab still reads "Tools" while the panel inside is headed "Components" — the rename was tried and reverted after a browser measurement (~78px of label against ~49px of room), and it belongs to WP 7.9, which reworks that rail. **WP 7.8 shipped 2026-08-22** (merged `8b423e7`, migration **v62**, full gate EXIT=0), and it is the one that turned the canvas from a picture into a measurement. Nine genuinely different relationships had been drawn as the same anonymous line; an edge now carries a **kind** — `triggers · then · contains · branch · uses` — held as **one** frozen legality table in `packages/shared/src/skill-flow-grammar.ts` that the projector, the connect handler and the tests all read, gated by a source scan that fails on a second declaration (the two `buildRunFilterWhere` copies were this repo's own lesson). An entry-point flow is now **forward reachability, not lane membership**, every reached node is labelled *always read* or *maybe read* **on its accessible name, not shading alone**, and the panel says it in words — *"always reads 4 sections, 1,240 tokens; may additionally read 1 file and call 1 tool, up to 3,900"* — with a test that independently recounts through the same `TokenCounter` so a second counting path breaks the equality. A file cited four times is **one box with four arrows**, so "how many files does this command read" is finally countable. Refusals became guidance: an impossible arrow does not snap and raises **no** error, a near-miss offers the move you meant in one click, and a genuine mistake names the rule and links the guide — gated over **all 92 ordered node-kind pairs**, with the old catch-all string banned by test. The single "unresolvable branch" in the whole registered corpus turned out to be a **mis-parse of two ordinary sentences**, now a named regression test. Box positions persist per **skill** (v62, one table) with an **Auto-arrange** reset, and are deliberately **not** written into `SKILL.md` — proved by a test asserting identical sha256 and **unchanged L2 body tokens**, because the body is exactly what the model reads and this app meters it. Two changes a reviewer should know: the trace aligner's gatekeeper inference was **widened** (once the mis-parse was gone, a gatekeeper with one plain successor could never be implied visited), and `layoutSkillLanes` was **deleted** rather than left as a second, quietly wrong answer. One documented deviation from the approved design: `uses → entry_point` is **legal and not authorable**, because the projector has emitted cross-flow `see /other` references since Skill IDE WP 1.2. **Still open in Phase 7: 7.9 only.** **And after four Studio work packages, nobody has still ever used any of this** — 7.8 opened no browser at all, headless or otherwise, so there is no screenshot and no rendered measurement in it: both themes, keyboard reach, whether the token sentence wraps and whether five dash patterns are distinguishable at real zoom are all unknown. The trace coupling was exercised through the real route/projector/aligner against **seeded** `run_steps`, never a live model against a bound MCP server, and no save has ever been completed against a running API. |
...
| **Platform hardening — first-run demo seed, in-app docs + changelog, redacted diagnostics bundle, migration upgrade-test harness, fleet-scale performance budgets, owner-acceptance consolidation** | 🚧 **3 of 6 built, MEDIUM (rolling)** — plan at [`planning/Roadmap/RM-18-platform/`](./planning/Roadmap/RM-18-platform/) (ledger: [`STATUS.md`](./planning/Roadmap/RM-18-platform/STATUS.md) — authoritative). **WP 1.6** consolidated every pending owner walk across all ledgers into one runnable checklist — [`owner-acceptance-consolidated.md`](./planning/Roadmap/RM-18-platform/owner-acceptance-consolidated.md), **192 boxes across 23 files**, not the ~90 that had been estimated. **WP 1.4** (2026-08-22) added the captured-fixture upgrade harness — six committed fixture databases, byte-deterministic, migrated forward under invariant assertions in the normal suite — **and it immediately found a live defect in shipped migration code**: v5 renamed `run_steps` out of the way on the recorded grounds that nothing FK-referenced it, and **v36 made that false**. SQLite rewrites a child's stored FK text to follow a renamed parent, so the rename repointed `run_feedback.step_id` at `run_steps_old`, which the rebuild then dropped — invisible to `foreign_key_check`, which inspects ROWS and the table was new. Reproduced independently: every insert failed with *"no such table: main.run_steps_old"*, **including one with a NULL `step_id`**, because SQLite resolves the FK target at PREPARE time. Fixed forward (v31's pattern) — **a database already damaged still needs a new numbered migration, and the owner's live database was deliberately not checked against it.** **WP 1.3** (2026-08-22) is the **diagnostics bundle**: `GET /api/diagnostics{,/markdown}` + a Settings row that shows the document before it is sent — versions, environment, database shape + migration level, recent errors, feature state; computed on read, **no migration, no table, no column, no dependency, no feature flag**. Secret-freedom is **structural, not filtered**: the environment group walks a 78-entry hard-coded catalogue of the variables `config/env.ts` recognises and emits `{ name, status }` only, so **no code path carries a value into the document** and no regex has to be trusted; user-typed free text (server names, skill titles, scenario labels, MCP commands) is never read — the bundle carries counts. A sentinel sweep seeds every recognised name and every real persistence path and fails on any leak. **One acceptance item did not hold and is on the record:** the errors group quotes system error text verbatim, so a `spawn … ENOENT` names the operator's configured command path. Deliberately **not stripped** (that would need a second redactor, which the WP forbids); the false claim was deleted, the preamble says to read that section first, and the boundary is pinned in **both** directions. Where no error source exists the bundle reads **not captured** — a discriminated union, so it cannot render as "zero errors". **Still open: WP 1.1** (first-run seed), **1.2** (in-app docs route) and **1.5** (fleet-scale perf pass). Both historic "blocked on Benchmarks P1/P3" flags were rechecked 2026-08-21 and were **false** — nothing in this item is blocked on another workstream. **No browser has been opened for any of it**: the Settings diagnostics row has no two-theme or keyboard walk. |
