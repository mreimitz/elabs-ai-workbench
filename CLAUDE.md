...
> [`roadmap/qlik-answers/STATUS.md`](./roadmap/qlik-answers/STATUS.md),
...
| **CI + lint** | ✅ Built (root `.github/workflows/ci.yml` runs typecheck/test/build/lint; Biome via `pnpm lint`/`pnpm format`, `biome.json`) |
| **Polished UI/UX** | ✅ Web UI rebuilt on the `@brand/*` design system (exposes 2 themes: `qlik-bright` default + `qlik-dark`, plus a **System** OS-preference option; per-action busy state, error surfacing, shared formatters/components); typecheck + build green |
...
| **MCP × model compatibility (limits & heatmap)** | ✅ Built (Phase 5 · `GET /api/scans/:id/heatmap`, `/api/compatibility/models`, `POST /api/runs/:runId/compatibility`; web `apps/web/src/features/compatibility/`). Trends/recommendations (WP 5.7) open — see `roadmap/testing/STATUS.md` |
...
| **Qlik Answers as a test target — `qlik_answers` provider kind (tenant assistants = models), dedicated run executor (thread → one-shot stream/invoke; sources + assistant-version Etag captured; questions-consumed as the cost unit, token counts estimated — the API reports no usage), clean-session invariant (never MCP servers/skills; incompatible members skipped), wizard auto-detect on Qlik Cloud tenants + free assistants probe → one-click provider/env setup (reuse server OAuth preferred, API-key fallback)** | ✅ Built (all 14 WPs, Phases 0–3 · migrations **v23** [credential link] + **v24** [`answers_mode`] · API 1511 tests). `qlik_answers` kind + additive wire (0.1); dual-auth resolver [linked MCP-server OAuth/headers **or** API key; "auth broken" state] + migration (0.2); assistants roster [**classic-filter deferred** — the public assistants API exposes no classic/agentic distinguisher; owner-decided ship-all] + Settings form (0.3); executor [thread → `invoke`/`stream`, AE-x → outcome (AE-4 → `stopped_guardrail`/`prompt_rejected`), estimated tokens, questions metric] behind the `RunService.execute()` branch [structural clean-session — no MCP session/tool/skill] + interactive `promptType:"thread"` turns (1.1–1.3); clean-session enforcement **3 layers** + D-QA6 incompatible-member skipping (1.4); per-provider concurrency cap + 429/AE-6 backoff (1.5); server-side Qlik detect + **list-only** availability probe [never invokes] (2.1); wizard "set up Qlik Answers" offer + server badge (2.2); env-editor conditionals + transport toggle (2.3); console sources panel + assistant-version (Etag) drift marker (3.1); cost surfaces [`answersQuestions` launch preview, KPI "est." by kind, suite question totals] (3.2). **Phase 5 (answer rendering, all 6 WPs)** reworked how an app-backed answer is rendered end-to-end: the console renders the answer as the assistant's own ORDERED Adaptive-Card block sequence (narrative `text` blocks interleaved with `Qlik.Snapshot` hypercube-data insets, D-QA8) instead of a flattened string, each snapshot carrying footnote-style citation chips back to its insight (D-QA9) and its actual hypercube table/KPI value (D-QA10); insights are **data-first** (the Sources panel leads with the hypercube table); the assistant's shown reasoning is parsed into structured phases (Understanding → Rewritten question → Asset search → Classification → Draft, D-QA11) instead of one opaque blob; the KPI rail + console chrome are kind-aware and **question-first** for `qlik_answers` (context-window surfaces gated off — the API reports no token usage); and the run report (JSON + Markdown) now carries the same `blocks` / snapshot `data` / citations — markdown renders them as tables + `[^n]` citation markers, and a LEGACY run (captured before this rework) gets them too via the same read-time `deriveLegacyAnswerStep` projection `GET /api/runs/:id` already used. Everything is **stub-tested behind an injectable fetch — no real tenant is ever contacted**; gate green throughout. Plan + locked decisions D-QA1–D-QA7 at [`roadmap/qlik-answers/`](./roadmap/qlik-answers/) (ledger [`STATUS.md`](./roadmap/qlik-answers/STATUS.md) — authoritative). **Owner-acceptance pending (needs a real Qlik Cloud tenant):** live probe/run/suite; both-theme + keyboard walks of every new surface (incl. Phase 5's answer/insights/reasoning rendering and the new report block tables); the D-QA7 classic-vs-agentic distinguisher; AE-4 reproduction; whether an MCP-server OAuth token authorizes the assistants API — all tracked in the ledger's Owner-acceptance section. Research: [`roadmap/research/qlik-answers-as-model.md`](./roadmap/research/qlik-answers-as-model.md) |
| **Unified Sessions — one session experience across the three run backends (shared terminal table via `stopReasonCode`, additive `ended` terminal + `seen`, stall-based clock [no default wall cap, 10-min stall + wait budget → `wait_expired`], persisted `phase` + queue visibility, capability manifest `capabilities_json`, one status module, SSE cursor resume, OpenAI-compatible facade around Qlik Answers as a parallel track)** | 🔜 Planned, **HIGH priority — runs FIRST** (decisions **D-US1–D-US15** locked 2026-07-16). Plan: [`roadmap/unified-sessions/`](./roadmap/unified-sessions/) (ledger: [`STATUS.md`](./roadmap/unified-sessions/STATUS.md) — authoritative; [`execution-plan.md`](./roadmap/unified-sessions/execution-plan.md) + [`kickoff-prompt.md`](./roadmap/unified-sessions/kickoff-prompt.md), Waves 1–5 with per-WP model tags). Research: [`research/unified-run-sessions/`](./research/unified-run-sessions/) (docs 00–04) |
| **Observability — fleet monitoring, search & issues on top of the Unified Sessions contract (metrics-over-time + SQLite-FTS5 search + `RunFilter` grammar + saved-views + human-feedback backbone [feedback never blends into grades, AR6 intact], Dashboard grows Scans \| Testing \| Issues tabs with chart→feed drill-down, runs-feed filter/search/views upgrade, console depth [wire-level step hierarchy `parentStepId`, per-step economics + hotspots, fork-from-step with `derivedFromRunId` lineage, in-run search], watch rules [on-terminal + windowed with catch-up-on-boot, in-app notification center + one generic webhook, promote-run-to-test], fleet issues extending the v26 rating-issues registry [deterministic clustering + auto-reopen on regression, opt-in LLM assist via the CLI-first judge chain, owner-initiated Assistant fix loop], pricing editor [DB + effective dates], custom chart composer, review-queue lite, scheduled digest)** | 🔜 Planned, **HIGH priority — starts after Unified Sessions Waves 1–3 merge** (D-OB27/28; contract-independent WPs may run earlier). Plan + locked decisions **D-OB1–D-OB28** at [`roadmap/observability/`](./roadmap/observability/) (ledger: [`STATUS.md`](./roadmap/observability/STATUS.md) — authoritative; [`kickoff-prompt.md`](./roadmap/observability/kickoff-prompt.md), 27 WPs across Phases 1–5 with a per-WP model map; superseded first-cut Phase 0 preserved at `_superseded/`). Research: [`research/langsmith-observability/`](./research/langsmith-observability/) |
| **Illustration design system — theme-token-driven isometric "3D blueprint" illustration components for the app's core entities (`packages/illustrations`: LLMs/agents, MCP servers, skills, runs, suites, …), colors derived live from `qlik-bright`/`qlik-dark` via a closed `--illus-*` token layer; machine-readable registry + in-app asset-repository gallery (`/illustrations`); declarative zod scene spec (the only composition path) + deterministic scene renderer; explain-mode step player for app-internal processes; assistant-composed scenes (describe a workflow in chat → validated spec → preview → save)** | 🔜 Planned, **HIGH priority** — research + plan + locked decisions D-IL1–D-IL17 at [`roadmap/illustrations/`](./roadmap/illustrations/) (ledger: [`STATUS.md`](./roadmap/illustrations/STATUS.md)); origin: the owner's Self-Learning Agentic Loop reference image; the repo-root `illustrations/` folder is export output only |
| **CI & headless automation — service tokens, `mcpfp` CLI (scan/suite/assert/report as a thin API client), server-side assertions with exit codes, baseline-delta PR-comment artifact, GitHub Actions packaging** | 🔜 Planned, **HIGH priority** — plan at [`roadmap/ci/`](./roadmap/ci/) (ledger: [`STATUS.md`](./roadmap/ci/STATUS.md)); suite gating blocked on Benchmarks Phase 3 |
...
This repo (`qlabs-ai-benchmark`) currently contains one app under `mcp-token-footprint/`.
...
- **UI / styling:** the upstream **`@brand/*` design system** (brand-ui v1.9.0, Radix + CVA),
  vendored as tarballs in `vendor/brand/` and consumed via `file:` deps. **Tailwind v4**
...
`.github/workflows/ci.yml` runs typecheck/test/build/lint on push + PR. The `@brand/*` packages
are vendored locally (not on npm); the agent-usage kit is at `vendor/brand-ui-agent-kit/`. The
`@brand/cli` is also vendored (`vendor/brand/brand-cli-1.9.0.tgz`, a root devDependency) — run it
with `pnpm exec brand-ui <cmd>` (`info`/`search`/`docs`) for the real component API. The old
hand-rolled `packages/brand-ui` adapter has been **removed** (it was retired and unused; the
`enforce-brand-ui` hook still blocks any `@mcp-token-footprint/brand-ui` import).
...
> **Hard rule — brand-ui only.** Every visible element comes from the `@brand/*` design system.
...
UI primitives come from **`@brand/ui`** (Button, Card, MetricCard, Dialog, Wizard, Sidebar,
AppShell, EmptyState/ErrorState, Badge, Alert, Descriptions, …), tables from **`@brand/data`**
(`DataTable`, `SearchInput`, `FilterBar`), icons from `lucide-react` / `@brand/icons`, theming from
**`@brand/tokens`**. Styling is **Tailwind v4 + semantic oklch tokens** (`bg-card`,
...
**Two themes** are exposed — `qlik-bright` (default) and `qlik-dark` — applied with `data-theme` on
`<html>` by `ThemeProvider` and switched in Settings. (`@brand/tokens` v1.9.0 also ships a
`blueprint` theme, but the app deliberately filters it out and coerces any persisted `blueprint`
back to `qlik-bright` — see `apps/web/src/lib/theme.ts`, `main.tsx`, and `SettingsView.tsx`.) New
UI must read correctly in both themes.

App-specific compositions of `@brand` parts live in `apps/web/src/components/` (`SelectField`,
`TokenViz`) and `apps/web/src/lib/table.tsx` (`col` helper). Code/text display uses `@brand/editor`
Monaco `CodeEditor` (web workers in `apps/web/src/main.tsx`) and read-only code components like
`features/testing/CodeSnippet.tsx`. When `@brand/*` lacks a component it's a real upstream gap —
compose from primitives or raise it; don't hand-roll. The authoritative component reference is
vendored at `vendor/brand-ui-agent-kit/` (manifest, `llms/`, `playbooks/`). The whole web UI was
rebuilt on `@brand/*`; `pnpm typecheck && pnpm build` are green.
...
- **No new runtime dependency without a reason** — prefer the existing stack. Owner approval for
  any new UI dependency or any `@brand/*`/brand-ui version change.
...
- `rules/brand-ui-only.md` — **the hard rule:** every visible element is a `@brand/*` component.
...
- `rules/dependencies.md` — pnpm workspace, vendored `@brand/*` tarballs, Tailwind v4, agent kit.
- `rules/library-first.md` — compose from `@brand/*`; when something's missing, raise the gap.
- `rules/styling-and-tokens.md` — Tailwind v4 + `@brand/tokens`, two themes, no raw colors.
