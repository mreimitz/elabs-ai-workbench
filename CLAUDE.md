...
- **Workbench MCP server** (`mcp-server/`) — `POST /api/mcp`, the app's OWN read-only MCP mount
  (stateless streamable HTTP; `GET`/`DELETE` answer 405). Contract in
  `packages/shared/src/workbench-mcp.ts`; feature-flagged by `mcp_server`.
