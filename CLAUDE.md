...
(`biome check`) and `pnpm format` (`biome format --write`). (Note: there is currently **no**
`.github/workflows/` in this repo — the gate is run locally.) `@elabs-ai/components-cli` is a root
