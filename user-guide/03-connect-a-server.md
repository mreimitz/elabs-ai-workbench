...
- **Type** — an optional category. You can leave it as **No type / Untyped**. Types are handy
  later when you want to filter or [compare](./05-compare.md) servers of the same kind.
...
> See [Troubleshooting](./14-troubleshooting.md) for details.
...
- **[Run a scan](./04-scan-and-read-footprint.md)** to measure its footprint.
...
![A connected server's detail view: its tabs, startup-token summary, automatic Findings ranked most-severe-first, and the per-tool token distribution.](./images/02-servers.png)
...
> up URLs from inside its container. See [Troubleshooting](./14-troubleshooting.md).

---

Next: [Scan and read the footprint →](./04-scan-and-read-footprint.md)
