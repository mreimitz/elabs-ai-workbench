...
[Scan and read the footprint](./04-scan-and-read-footprint.md)). It opens the Compare view with
...
![Comparing two scans of a server: the token delta at the top, and a per-tool table of Before, After, and change for every matched tool.](./images/05-compare-scans.png)

## One important guardrail

The app **will not silently compare two scans that were counted with different
[token profiles](./01-key-concepts.md)** — the numbers wouldn't be comparable. If your two
...
Next: [Run a tool →](./06-run-a-tool.md)
