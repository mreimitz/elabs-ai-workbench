...
> [Settings](./13-settings.md) for where it lives and how to maintain it.
...
![The AI Workbench interface: the grouped sidebar on the left, the top bar with search and theme, and the Dashboard in the middle.](./images/01-dashboard.png)
...
- **Assistant** — a general-purpose, multi-model assistant with chat, research, and multi-agent
  **mission** modes (see [Assistant](./16-assistant-hub.md)). This is different from the **App
...
- **Skills** — register and inspect Agent Skills (see [Skills](./08-skills.md)).
...
![The Dashboard Issues tab: tracked issues filed against skills and servers, most severe first.](./images/24-dashboard-issues.png)

## Your first five minutes

The quickest way to see what the app does:

1. Go to **MCP Servers** and [add a server](./03-connect-a-server.md).
2. **Test** the connection to confirm it works.
3. **Scan** it, then [read the footprint](./04-scan-and-read-footprint.md) — the total plus
   every tool ranked by cost.
4. Open a tool and **[run it](./06-run-a-tool.md)** to see what a real call costs.
5. **[Export a report](./07-reports.md)** to save the results.

The next page walks through step 1 in detail.

---

Next: [Connect a server →](./03-connect-a-server.md)
