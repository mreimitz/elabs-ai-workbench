...
surface than the [App assistant](./12-assistant.md) dock (labeled **"App assistant"** in Settings).
...
setup — if you've already used [Testing](./09-testing.md) or [Skills](./08-skills.md), the
...
![The Assistant workspace: the conversation in the centre and the Session meta rail (Progress, Outputs, Context) on the right.](./images/25-hub-workspace.png)
...
![The Agents & Crews directory: a card grid of saved agents, each with its model, tools, skills, and run history, organized into crews.](./images/26-hub-agents.png)
...
![The Org chart: each crew as a tinted group box with its execution topology drawn between agent nodes.](./images/27-hub-org.png)
...
token-footprint counters — [Skills](./08-skills.md) covers where those numbers come from. Skills
...
finish the wizard like any other [stdio (local command) server](./03-connect-a-server.md).
