// ==================================================================================================
// The runtime cast — its own census, beside its own module
// ==================================================================================================
...
import { ILLUSTRATION_RUNTIME_CAST } from "./cast-runtime.js";

describe("the runtime cast (WP 1.1)", () => {
  it("declares exactly these ids, and nothing else", () => {
    assert.deepEqual(
      ILLUSTRATION_RUNTIME_CAST.map((member) => member.meta.id).sort(),
      [
        "model",
        "prompt",
        "provider",
  ...
  it("keeps every member's component keyed to its own entry", () => {
    for (const member of ILLUSTRATION_RUNTIME_CAST) {
