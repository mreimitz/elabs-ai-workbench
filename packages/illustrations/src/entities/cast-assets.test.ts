// ==================================================================================================
// The assets cast — its own census, beside its own module
// ==================================================================================================
...
import { ILLUSTRATION_ASSETS_CAST } from "./cast-assets.js";

describe("the assets cast (WP 1.2)", () => {
  it("declares exactly these ids, and nothing else", () => {
    assert.deepEqual(
      ILLUSTRATION_ASSETS_CAST.map((member) => member.meta.id).sort(),
      [
        "feedback-report",
        "file",
        "prompt-template",
  ...
  it("keeps every member's component keyed to its own entry", () => {
    for (const member of ILLUSTRATION_ASSETS_CAST) {
