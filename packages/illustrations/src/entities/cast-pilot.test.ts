// ==================================================================================================
// The pilot cast — its own census, beside its own module
// ==================================================================================================
...
import { ILLUSTRATION_PILOT_CAST } from "./cast-pilot.js";

describe("the pilot cast (WP 0.3)", () => {
  it("declares exactly these ids, and nothing else", () => {
    assert.deepEqual(
      ILLUSTRATION_PILOT_CAST.map((member) => member.meta.id).sort(),
      [
        "agent",
        "mcp-server",
        "skill",
  ...
  it("keeps every member's component keyed to its own entry", () => {
    for (const member of ILLUSTRATION_PILOT_CAST) {
