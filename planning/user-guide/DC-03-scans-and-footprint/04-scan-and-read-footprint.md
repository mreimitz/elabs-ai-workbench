---
type: "Guide Page"
title: "4. Scan and read the footprint"
description: "A scan is the core measurement in this app. It connects to a server, pulls in everything"
tags: ["documentation", "DC-03"]
...
the server offers, and calculates the token cost — the [startup footprint](../DC-01-getting-started/01-key-concepts.md).
This page explains how to run one and how to read the results.

![A completed scan: the summary metrics (total footprint, tools, average and largest tool) above every tool ranked by token cost, with its name/schema/description split.](../DC-23-product-overview/images/03-scan-footprint.png)
...
[token profile](../DC-01-getting-started/01-key-concepts.md) and saves the result to your scan history.
...
- **Diff vs previous** — jumps straight to a comparison with this server's previous scan, so
  you can see what changed (more in [Compare](../DC-04-comparison/05-compare.md)).
- **Export** — saves this scan as a report (see [Reports](../DC-06-reports/07-reports.md)).
...
[Run a tool](../DC-05-tool-playground/06-run-a-tool.md).
...
Next: [Compare →](../DC-04-comparison/05-compare.md)
