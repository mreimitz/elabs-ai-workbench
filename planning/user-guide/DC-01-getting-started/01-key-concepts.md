...
description: "This page explains the ideas the rest of the guide builds on. If you already work with MCP"
...
You'll choose one of these when you [connect a server](../DC-02-mcp-servers/03-connect-a-server.md).
...
and include it in a test run to see its real effect (see [Skills](../DC-07-skills/08-skills.md)).
...
and what it cost. The app lets you drive real sessions (see [Testing console](../DC-08-testing-console/09-testing.md)),
...
- **Startup footprint** — the token cost of the tool/resource/prompt *definitions*, loaded
  when the server connects. This is what a [scan](../DC-03-scans-and-footprint/04-scan-and-read-footprint.md) measures.
- **Runtime cost** — the token cost of actually *calling* a tool: the request you send plus the
  response that comes back. This is what the [tool run console](../DC-05-tool-playground/06-run-a-tool.md) measures.
