...
> The live plan/ledger is [`../../roadmap/skills/STATUS.md`](../../Roadmap/RM-24-skills/STATUS.md).
> **Phase 2 — attaching skills to test scenarios ([`08-scenario-attachment.md`](./outputs/08-scenario-attachment.md),
...
1. [`00-goals-and-scope.md`](./notes/00-goals-and-scope.md) — the request decomposed into concrete
...
2. [`01-agent-skills-format.md`](./notes/01-agent-skills-format.md) — ground truth on the Agent Skills
...
3. [`02-current-architecture-map.md`](./notes/02-current-architecture-map.md) — how MCP servers work
...
4. [`03-data-model.md`](./outputs/03-data-model.md) — the new tables (`skills`, `skill_versions`,
   `skill_blobs`, `skill_files`, `scenario_skills`), DDL, the content-addressed blob store, and how
   migrations are applied.
5. [`04-versioning-and-diff.md`](./outputs/04-versioning-and-diff.md) — the version model (uploaded vs
   GitHub), change detection, the full-tree diff algorithm, rename detection, and the token-delta
   "what changed" comparison.
6. [`05-api-surface.md`](./outputs/05-api-surface.md) — the new `/api/skills*` routes, request/response
   shapes, and the shared types + zod schemas to add first (contract-first).
7. [`06-ingestion-and-github.md`](./outputs/06-ingestion-and-github.md) — the ingestion pipeline: zip/file
...
8. [`07-ui-plan.md`](./outputs/07-ui-plan.md) — the **Skills** side-menu item, `SkillsView` + `SkillRail`,
...
9. [`08-scenario-attachment.md`](./outputs/08-scenario-attachment.md) — Phase 2: `scenario_skills`,
   `AllowedSkill`, the `AddSkillModal` picker, run-engine wiring, and the token-footprint accounting
   for an attached skill.
10. [`09-implementation-plan.md`](./outputs/09-implementation-plan.md) — sequenced work packages, the three
    new runtime dependencies and why, the quality gate, and a definition of done per WP.
11. [`10-open-questions.md`](./notes/10-open-questions.md) — the decisions that need your call before or
    during build (chief among them: what "attaching a skill to a scenario" should actually feed the
    agent under test).
12. [`schema/`](./outputs/schema/) — machine-readable artifacts: the parsed-manifest JSON schema and the
    work-package ledger.
