...
- [ ] WP 7.3 — Skill-settings panel + one draft store / "Save as vN" (I3/I7, SI3/SI8; Overview goes read-only) · Depends 7.1 · Domain: studio/settings + draft store · Size L · **SI1 slice already shipped as 7.3a below** · **inherits one line from 7.1 (2026-08-21):** Overview's **"Save as new version"** button is still on the page. 7.1 left it deliberately — removing it before this WP's settings panel exists would strand Overview's keyword editor with no way to save. Delete it here, with the keyword editor, as this WP's own step already says
...
- [ ] WP 7.4 — Editable files, multi-tab editor; Inspector Files = browse-only, Save/Discard bar deleted (I5, SI2) · Depends 7.3 · Domain: studio/files · Size M–L · **deferred this session (needs SkillInspector/Overview — in-flight WP 6.5's territory)** · **inherits one line from 7.1 (2026-08-21):** the Inspector's Files tab still carries its **Discard / Save…** bar, because 7.1 made the Studio's Files rail READ-ONLY — deleting the bar first would leave no way to edit a file at all. It goes when this WP makes files editable in the Studio. Also from 7.1: `?file=` already round-trips and drives the rail selection, but the centre still hosts only `SKILL.md` — the multi-tab editor is this WP
