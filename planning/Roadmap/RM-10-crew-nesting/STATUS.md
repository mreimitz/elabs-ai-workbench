---
type: "Status Ledger"
title: "crew-nesting \u2014 work-package status ledger"
description: "Living state for the crew-nesting plan, read and updated by the next-wp skill (and the /next-wp"
tags: ["roadmap", "RM-10"]
...
**Plan:** [`README.md`](./item.md) (mission + D-CN log + WP index + dependency graph) ·
...
## Decision log
_Entries: date · decision · rationale._ Kickoff locks **D-CN1–D-CN10** in [`README.md`](./item.md) §3.
...
- [ ] **Both-theme (`light` + `dark`) + keyboard walk** of: the N-level org chart/rail (arrow/Tab/Enter roving, cycle placeholders), the nested **mission-board drill** (the first `Dialog`-in-`Dialog` in the app — focus trap + ESC ordering), the crew-editor **Sub-crew add path** + cycle warning, and the **per-level cost meter** — accepted: ____
