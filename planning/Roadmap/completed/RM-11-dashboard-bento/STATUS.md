...
> Plan + locked decisions D-DB1–D-DB3 in [`README.md`](./item.md); shared rules in
...
- [x] WP 2.4 — footprint lines differentiate by STROKE, not colour alone (D-DB4) — done 2026-08-20 · `3fb9a19` · spec [`WP-2.4-series-differentiation.md`](./WP-2.4-series-differentiation.md). Each line takes its pattern from upstream's own `seriesDashArray`; colour is unchanged, and the legend swatch is an SVG line drawing the same stroke AND dash as its series so the two cannot disagree. **The trap:** `Line`'s `dashArray` styles only the segment *after* `dashFromIndex` (the pair exists for forecast styling), so a `dashArray` alone is **silently inert**; `dashFromIndex={0}` clears upstream's own `dashFromIndex >= 0 && dashFromIndex < data.length - 1` bound and dashes the whole line. Both props go together or neither does — `Line`'s `dashArray` default is `"6 4"`, so a lone `dashFromIndex={0}` would dash the series meant to be solid. Faithful-stub test pins both, **mutation-verified** (dropping `dashFromIndex` fails with `expected undefined to be +0`). `seriesDashArray` is imported at the call site rather than re-exported through `lib/chart-colors.ts`: that module documents itself as *pure and framework-free*, and making it depend on the charts barrel broke every suite that mocks the package — a real constraint discovered by breaking it. Verified in-browser both themes: `chart-1` solid then `"6 4"`, `"2 3"`, `"8 3 2 3"`, `"1 4"`, `"10 5"`, `"4 2 1 2"` — 7 distinct patterns, legend matching the chart, 0 console errors.

**Phase 2 complete.** Gate fully green at `3fb9a19`: typecheck 0 · shared/cli/api **3564 pass / 0 fail** · web **337 files / 3615 passed** · build 0 · lint 0. The 7 api compatibility failures and the 2 Biome file-size errors this ledger recorded as pre-existing on `main` are both resolved.
