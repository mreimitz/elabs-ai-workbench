...
title: "WP 3.8 \u2014 Compare (test \u00d7 scenario)"
description: "Phase: 3 \u00b7 Size: M \u00b7 Depends on: 3.6, 2.2"
tags: ["roadmap", "RM-26"]
...
## Design
- A `@elabs-ai/components-data` `DataTable` of `CompareRow` for one test across scenarios: columns = scenario, model,
  tokens ↑/↓, tool calls, **peak context %**, est. cost, outcome (`StatusBadge`; overflow flagged),
  duration. `tabular-nums`; sortable.
- An **overlay** of each run's context curve (small multiples, or one overlaid `@elabs-ai/components-charts` chart
...
- The curve overlay reads in both themes (light, dark); each row opens its run.
