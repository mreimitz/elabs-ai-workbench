 ...
 *    names *statically*, so `` `bg-chart-${n}` `` is never seen by the scanner: those utilities are
 *    only generated at all because unrelated files happen to spell `bg-chart-1`…`bg-chart-5` as
 *    literals, and `bg-chart-6`…`bg-chart-12` are generated by nothing. {@link chartSwatchStyle}
 *    sidesteps the scanner entirely and — the real win — guarantees a swatch and its series are
 *    painted from the *same* token string.
 *
 ...
 *
 * Use this instead of a `` `bg-chart-${n}` `` class: Tailwind cannot generate a class it never sees
 * as a literal, and this keeps the swatch and the plotted series on one token by construction.
 */
