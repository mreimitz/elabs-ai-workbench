...
describe("chartSwatchStyle", () => {
  ...
  test("covers the whole ramp, including the slots no `bg-chart-N` class exists for", () => {
    // Tailwind extracts class names statically, so `bg-chart-6`…`bg-chart-12` are generated by
    // nothing in this app — an inline style is the only reliable way to reach them.
