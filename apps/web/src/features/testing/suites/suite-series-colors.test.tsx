 ...
 * 2. **The legend swatches may never have been painted at all.** They were a *template-literal*
 *    Tailwind class, `` `bg-chart-${n}` ``. Tailwind extracts class names STATICALLY, so it never
 *    sees that string: `bg-chart-1`…`bg-chart-5` are generated only because unrelated files
 *    (`TokenViz`, `ContextChart`) spell them as literals, and `bg-chart-6`…`bg-chart-12` are
 *    generated by nothing. Re-moduloing alone would have produced *invisible* swatches past the 5th
 *    environment. Both legends now paint from an inline `var(--chart-N)` style instead.
 *
...
describe("SuiteScatter series colours", () => {
  ...
  test("paints legend swatches from the same tokens — as an inline style, not a dynamic class", () => {
    ...
    // No swatch may carry a `bg-chart-*` class — Tailwind cannot generate one built by template
    // literal, so such a swatch would render transparent past the 5th environment.
