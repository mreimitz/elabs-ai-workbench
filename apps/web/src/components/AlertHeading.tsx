 ...
 *
 * WHY IT EXISTS
 *   `@brand/ui`'s `AlertTitle` (vendor `alert.tsx`) always renders a literal `<h5>`, regardless of
 ...
 *   `SectionCardTitle.tsx` already applied to `@brand/ui` `CardTitle` (D-IC5).
 *
 * WHAT IT IS
 ...
 *   `@brand/ui` accepts a heading level upstream.
 *
 * Every visible element stays a semantic-token utility (no raw colour); `className` is layout-only.
 * Reads correctly in both `qlik-bright` and `qlik-dark` (same tokens `AlertTitle` uses).
 */
...
export interface AlertHeadingProps extends HTMLAttributes<HTMLHeadingElement> {
   ...
   * @default 5 — matches `@brand/ui` AlertTitle's own hardcoded `<h5>`.
   */
...
export const AlertHeading = forwardRef<HTMLHeadingElement, AlertHeadingProps>(
  function AlertHeading({ level = 5, className, ...props }, ref) {
    ...
    // Classes mirror `@brand/ui` AlertTitle exactly (vendor alert.tsx) — same look, correct level.
    return (
