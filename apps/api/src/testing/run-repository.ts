...
import { buildRunFilterWhere } from "../observability/run-filter-sql.js";
...
// AM-OB4 — the RunFilter -> SQL translation used to be DUPLICATED here, byte-for-byte, from
// `observability/metrics.ts`, and only THIS copy was pinned to the shared `matchesRunFilter`
// predicate by the cross-check. It is now imported: one definition, one place to fix a filter bug,
// and the runs feed and the charts can no longer answer different questions from the same filter.
// `q` (FTS) is still ANDed in by `queryRuns` above -- it was never part of the builder.
