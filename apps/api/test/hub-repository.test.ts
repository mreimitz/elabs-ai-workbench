...
test("migration v47 — a fresh DB carries every hub_* table (schema.ts baseline)", () => {
  ...
  assert.equal(
    ...
    55,
    "LATEST_SCHEMA_VERSION auto-derived to 55 (v47 = the 13 Assistant Hub hub_* tables, WP0.2; v48 = hub_session_skills, WP2.4; v49 = hub_memory.scope/scope_id + hub_agents.display_name + hub_crews.color + hub_sessions.archived_at, Assistant Hub UX WP1.0s; v50 = hub_sessions.tool_scope_json, end-user UX pass; v54 = hub_missions.parent_mission_id/depth/root_mission_id, crew-nesting mission-tree lineage; v55 = hub_sessions/hub_agents.provider_credential_id, model identity D-MI1)",
  );
...
test("migration v47 — a pre-v47 (v46) DB gains every hub_* table; neighboring rows survive; idempotent", () => {
  ...
  assert.equal(
    ...
    55,
    "stamped to LATEST (55) after v47…v55",
  );
  for (const table of HUB_TABLES) {
...
test("migration v49 — a fresh DB carries the 4 Wave-1 hub_* columns (schema.ts baseline)", () => {
  ...
  assert.equal(
    LATEST_SCHEMA_VERSION,
    55,
    "LATEST_SCHEMA_VERSION auto-derived to 55 by v55 (hub_sessions/hub_agents.provider_credential_id, model identity D-MI1)",
  );
  assert.equal(db.pragma("user_version", { simple: true }), 55, "fresh DB stamped at 55");
...
test("migration v49 — a pre-v49 (v48) DB gains the 4 columns; existing hub_memory rows read scope='profile'; idempotent", () => {
  ...
  assert.equal(
    db.pragma("user_version", { simple: true }),
    55,
    "stamped to LATEST (55) after v49…v55",
  );
...
test("migration v50 — a fresh DB carries hub_sessions.tool_scope_json; upgrade adds it, idempotent", () => {
  ...
  applyMigrations(db);
  assert.equal(db.pragma("user_version", { simple: true }), 55, "stamped to LATEST (55) after v50");
  assert.ok(
    columns(db, "hub_sessions").includes("tool_scope_json"),
...
test("migration v54 — a fresh DB carries hub_missions.parent_mission_id/depth/root_mission_id; LATEST is 55", () => {
  ...
  assert.equal(
    LATEST_SCHEMA_VERSION,
    55,
    "LATEST_SCHEMA_VERSION auto-derived to 55 (v54 = hub_missions.parent_mission_id/depth/root_mission_id, crew-nesting mission-tree lineage; v55 = hub_sessions/hub_agents.provider_credential_id, model identity D-MI1)",
  );
  assert.equal(db.pragma("user_version", { simple: true }), 55, "fresh DB stamped at 55");
...
test("migration v54 — a pre-v54 (v53) DB gains the 3 lineage columns; a legacy mission reads back parent=undefined/depth=0; idempotent", () => {
  ...
  assert.equal(db.pragma("user_version", { simple: true }), 55, "stamped to LATEST (55) after v54");
  const cols = columns(db, "hub_missions");
