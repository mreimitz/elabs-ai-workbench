...
test("migration v26 — a pre-v26 DB (stamped 25, no tables) is brought forward additively", () => {
  ...
  assert.ok(!tableExists(db, "rating_issues"));
  applyMigrations(db);
  assert.equal(db.pragma("user_version", { simple: true }), 60, "stamped forward to LATEST (60)");
