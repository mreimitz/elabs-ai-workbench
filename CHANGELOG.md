...
## Unreleased — the shell repairs came home, and the tool playground stopped asking for JSON

Three things followed the 4.1.0 upgrade.

**The window layout.** The library's new flagship app shell was designed by reading this app, so the
repairs this repo invented now ship upstream. The hand-rolled skip link, the ⌘K search button, the
active-menu-item highlight and the collapsed-menu label fix are all deleted here and imported
instead. Nothing looks different: the highlight was checked by measuring the two colour tokens
involved and finding them identical in both themes, and the collapsed rail was checked by eye.

**A real accessibility defect, fixed.** The assistant panel was rendered *inside* the main content
region — a side landmark nested in the main one, so a screen-reader user found the assistant within
the page rather than beside it. It is now a sibling, with a test that fails if it ever moves back;
the test was proved by putting it back and watching it go red. A second defect turned up while
comparing our page frame with the library's: scrolling regions had no keyboard focus, so a page whose
content held nothing clickable could not be scrolled by keyboard at all. Both scroll regions are now
reachable, with a visible focus ring.

**The tool playground.** Running an MCP tool used to mean typing raw JSON for anything list- or
object-shaped, because the form was generated by pattern-matching a type label. It now builds real
controls from the tool's own schema: lists get add/remove rows, fixed choices get a dropdown, numbers
get steppers, and each field shows its own documentation. All 24 tools on the workbench's own MCP
server render this way. Where a schema uses something the generator cannot represent faithfully, the
raw editor comes back **and says why** — including one case where the generator would have flattened
nested arguments and sent the wrong shape, which is refused outright rather than attempted.

**The comparison screen** gained its first chart: per-tool token changes drawn as before/after tracks,
with the table kept underneath as the record. Building it found two rendering faults that no test
could see — half the rows were being cut off, and long tool names were clipped at the start — both
measured and fixed.

Ten new chart types were reviewed against the screens that might use them; nine were rejected on
evidence, and the reasons are recorded. Plan and per-item state:
