...
Compare/Usage/KPI), live theme switch, icon-hover at scale. Carry-forward for the owner / upstream `@brand/*`:
- **IconButton-in-Dialog Escape hazard** — a modal auto-focusing an `IconButton` opens its tooltip, which
  ...
  primitive-level fix).
- **`@brand/data` `FacetFilter` `h-26` vs `@brand/ui` `h-30`** — a ~2px residual on every mixed toolbar row
  (the diagnosed label-above cause IS fixed; this is the vendored floor). Upstream `@brand` report.
