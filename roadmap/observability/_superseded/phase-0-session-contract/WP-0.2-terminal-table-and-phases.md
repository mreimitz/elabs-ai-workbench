...
- New `apps/api/src/testing/session-terminal.ts`: `terminalFor(cause) → {status, outcome,
  ...
  `qlik-answers-executor.ts`. Fixes verified divergences: qlik 30-min deadline must terminate
...
- Phase events (from 0.1): subscription executor emits `phase: queued` (+ best-effort position
  in `detail`) BEFORE `gate.acquire()`; all executors emit `starting`/`running`; engine + qlik
...
- `apps/api/src/testing/engine.ts`, `apps/api/src/testing/claude-subscription-executor.ts`,
  `apps/api/src/testing/qlik-answers-executor.ts`, `apps/api/src/testing/run-service.ts`
...
- [ ] Qlik deadline no longer maps to `aborted` (regression test cites the old behavior).
