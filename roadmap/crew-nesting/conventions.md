...
[`../testing/conventions.md`](../testing/conventions.md) and applies verbatim; this file adds the
...
- If a WP needs a migration, **claim the next free `PRAGMA user_version` at claim time** — check
  [`apps/api/src/db/database.ts`](../../apps/api/src/db/database.ts) *and* every sibling
