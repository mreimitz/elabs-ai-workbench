...
Living state for the `assistant-hub` plan ([README](./README.md) ·
...
WP impact map). Evidence: [`research/agentic-session-sota/`](../../research/agentic-session-sota/) docs 00–03.**
...
- [x] WP 2.2 — crews + topologies (pipeline/debate/best-of-N + mission graph; Opus) — done 2026-07-18 · wp/assistant-hub/2.2 (integrated @ bb89591; blind best-of-N judge; @brand/flow mission graph)
...
- [x] WP 3.5 — artifact diff + review workflow (Sonnet) — done 2026-07-18 · wp/assistant-hub/3.5 (integrated @ f1d10c4; `@brand/editor` DiffEditor + critic → `AI/ChangeReview` accept/reject → new version + revert). **Owner-acceptance:** live critic run (provider key).
...
- [ ] **LOW-a11y (web `SessionRail.tsx:108`)** — raw `<button>` (no `brand-ui-allow`) → use `@brand/ui`
