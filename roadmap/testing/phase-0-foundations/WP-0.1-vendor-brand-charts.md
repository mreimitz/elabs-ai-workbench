# WP 0.1 — Vendor `@brand/charts`

**Phase:** 0 · **Size:** S · **Depends on:** — · **Owner action:** supply the tarball.

## Objective
Make charting available to the web app the same way the other `@brand/*` packages are, so the
...
## Why / references
`@brand/charts` exists upstream but is **not** vendored today (`.claude/rules/dependencies.md`). UI
concept §4 Zone B and §11 (component gaps) require it. Charting must come from `@brand/*`, not a
...
2. Add to `apps/web/package.json` dependencies, mirroring the existing entries exactly:
   ```json
   "@brand/charts": "file:../../vendor/brand/brand-charts-1.0.0.tgz"
   ```
3. In `apps/web/src/styles/app.css`, next to the existing `@source` directives for `@brand/ui`,
   `@brand/data`, `@brand/icons`, add:
   ```css
   @source "../../node_modules/@brand/charts/dist";
   ```
...
## Acceptance
- A throwaway chart (e.g. a `LineChart`/area component from `@brand/charts`) renders on a scratch
  view using only `--chart-1..5` tokens, and reads correctly in both themes (qlik-bright, qlik-dark).
...
## Risks
- If `@brand/charts` can't render a **streaming stacked-area with a horizontal limit line** cleanly,
