# Interface Craft — upstream `@brand/*` gaps

Four fixes in this plan land in code the repo **vendors** from `@brand/*` (brand-ui 1.9.0). Per the
...
fix. File the upstream issues against `mreimitz/qlabs-components` at the next bump.

Each entry: the gap · the app-side override this plan ships · the deletion trigger.

---

## 1. Failing on-fill token pairs (D-IC1) — `@brand/tokens`

- **Gap.** In `themes.css`, three `--<role>` ⇄ `--<role>-foreground` fill pairs fail WCAG AA 4.5:1 at
  the sizes they render (11–13px): `qlik-bright` `--primary` 4.31, `--success` 4.31, `--info` 3.76;
  `qlik-dark` `--destructive` 3.02. Each theme was tuned independently with no shared on-fill check, so
...
## 3. `CardTitle` has no `as`/`level` (D-IC5) — `@brand/ui`
...
- **Delete when.** `@brand/ui` `CardTitle` accepts an `as`/`level` prop; replace the wrapper with the
  prop at call sites.

## 4. `SelectTrigger` clips its value with no recovery (D-IC10) — `@brand/ui`
...
- **Delete when.** `@brand/ui` `SelectTrigger` carries a `title` (or exposes the selected label for
  one) by default.

## 5. `CardDescription` has no measure cap (D-IC9) — `@brand/ui`
...
- **Delete when.** `@brand/ui` prose components (`CardDescription`, `AlertDescription`) cap their
  measure by default (or expose a `prose`/measure variant the app can use without a wrapper).

## 6. `CardDescription` silently drops `text-muted-foreground` (found during WP 1.4) — `@brand/ui`

- **Gap.** `@brand/ui`'s vendored `CardDescription` composes its classes such that a `tailwind-merge`
...
- **File upstream** so `@brand/ui` `CardDescription` keeps its muted foreground under the
